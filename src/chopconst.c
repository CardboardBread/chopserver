...
int init_client(struct client *target, size_t window) {
	...
	// initialize pairing table
	hash_table_t *table = table_create(window);
	if (table == NULL) {
		DEBUG_PRINT("table_create");
		return -errno;
	}
...
int destroy_client(struct client **target) {
	...
	// cleanup pair table
	if (!table_destroy(old->pair_table, NULL)) {
		DEBUG_PRINT("table_destroy");
		return -errno;
	}
